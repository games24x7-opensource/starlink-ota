...
const getLogStreams = () => {
    ...
    if ('development' === NODE_ENV || 'docker' === ENV || /k8s/gi.test(ENV)) {
        streams.push({ stream: process.stdout, level: 'info' }, { stream: process.stderr, level: 'error' });
    }
...
function getCallSite() {
    const stack = (0, stack_trace_1.get)();
...
function parseErrorStack(error) {
    ...
    const traces = (0, stack_trace_1.parse)(error);
