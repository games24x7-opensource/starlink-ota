...
function onZKClientConnected() {
    ...
    for (let i = arrWatches.length - 1; i >= 0; i--) {
        client.fetchProperty(arrWatches[i].path, arrWatches[i].updateCallback, arrWatches[i].updateCallback);
    }
}
/**
...
function fetchProperty_Internal(propertyPath, callback) {
    zk.getData(propertyPath, function (err, data, stat) {
        if (err) {
            logger.error(err, 'Failed to fetch property from zookeeper: ' + propertyPath);
            return callback(err);
        }
...
client.fetchProperty = function (propertyPath, callback, updateCallback) {
    ...
    zk.exists(propertyPath, function (watcherEvent) {
...
client.fetchNode = function (nodePath, callback) {
    ...
    function (watcherEvent) {
        ...
        for (let i = 0; i < node.length; i++) {
            (function (i) {
                client.fetchProperty(nodePath + '/' + node[i], function (err, property) {
                    if (err) {
                        callback(err);
                    }
