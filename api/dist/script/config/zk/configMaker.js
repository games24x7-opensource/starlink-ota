...
console.log('configmake!!!!!!!!!!!!' + process.env.ZK_URL);
try {
    console.log(process.env.ZK_URL);
...
const maker = {
    init: function (appCallback) {
        const self = this;
        console.log(zkConnectionString);
        ...
        if (zkConfig.global.length > 0) {
            zkConfig.global.forEach(function (config, i) {
                ...
                const taskFunc = function (tasksCallback) {
                    zkClient.fetchProperty(config.path, function (err, data) {
                        if (err) {
    ...
    _fetchLocalProps: function (asyncCallback) {
        ...
        if (zkConfig.local.length > 0) {
            zkConfig.local.forEach(function (config, i) {
                ...
                const taskFunc = function (tasksCallback) {
                    zkClient.fetchProperty(zkConfig.basePath + config.path, function (err, data) {
                        if (err) {
                            logger.error(err, 'Config Maker Error.');
                        }
                        else {
    ...
    _fetchUUIDProps: function (asyncCallback) {
        ...
        if (zkConfig.local.length > 0) {
            zkConfig.local.forEach(function (config, i) {
                ...
                const taskFunc = function (tasksCallback) {
                    zkClient.fetchProperty(UUID_path + config.path, function (err, data) {
                        if (err) {
                            logger.error(err, 'Config Maker Error.');
                        }
                        else {
...
//# sourceMappingURL=configMaker.js.map