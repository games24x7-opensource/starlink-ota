...
4.  **Complete Clean Installation**: For a complete clean installation, including cleaning existing dependencies, installing with pnpm, and building all packages, use the setup script:
...
6.  **Build and Run the Server:**
    If you did not use the setup script, first build all packages:

    ```bash
    pnpm build
    ```

    Then, from the root directory, you can use the following commands:
...
```bash
# Full setup (clean, install, build all packages)
./scripts/setup-monorepo.sh
...
The OTA Dashboard is a modern React-based web interface for managing starlink-ota deployments. It comes with built-in dark and light modes from day zero, offering a versatile user experience.

![Dashboard Light Mode](./assets/dashboard_light.png)

<video controls src="./assets/dashboard_live.mp4" width="100%"></video>
